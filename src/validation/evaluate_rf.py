...
def create_csv(out_path: str = './', img_list: list[str] = None):
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        for img_path in img_list:
            ...
            img_class = img_path.split('\\')[-1].split('_')[0] # works on windows only
            if img_class == "average" or img_class == "Average":
                img_class = "Normal"
...
def evaluate_models(x, y):
    rf_model = None
    with open('../training/models/body_language.pkl', "rb") as f:
